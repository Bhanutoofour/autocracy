...
import HomeMediaCoverageSection from "./_components/HomeMediaCoverageSection";
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-black/10 bg-white">
      ...
      <div className="site-container flex h-[72px] items-center justify-between">
        <Logo />
        <nav className="hidden h-6 items-center gap-6 font-[var(--font-roboto-condensed)] text-[14px] font-semibold uppercase leading-5 tracking-normal text-[var(--ink)] lg:flex">
          {nav.map((item) => (
            <a
      ...
      <div className="site-container">
        ...
        <div className="grid grid-cols-2 gap-4 sm:gap-[30px] lg:grid-cols-4">
          {industries.map((industry, index) => (
            <Link
              className="group relative block h-[220px] overflow-hidden bg-[#312e33] sm:h-[320px] lg:h-[340px]"
              href={industry.href}
            ...
            >
              ...
              <h3 className="absolute inset-x-0 bottom-0 max-w-full p-4 pb-5 align-bottom font-[var(--font-roboto-condensed)] text-[20px] font-semibold leading-[1.08] tracking-normal text-white [overflow-wrap:anywhere] sm:p-5 sm:text-[24px] sm:leading-[1.16]">
...
function Strengths({
  ...
  return (
    <section className="bg-white py-16 lg:py-[72px]">
      <div className="site-container">
        <h2 className="max-w-[640px] font-[var(--font-roboto-condensed)] text-[38px] font-bold leading-[1.18] tracking-normal text-[#0a0a0b] lg:text-[48px] lg:leading-[1.2]">
        ...
        <div className="mt-24 grid gap-12 lg:grid-cols-3 lg:gap-[86px]">
          {items.map((item) => (
            <article className="pt-0" key={item.title}>
              ...
              <h3 className="max-w-[420px] font-[var(--font-roboto-condensed)] text-[24px] font-bold leading-[1.25] tracking-normal text-[#111113]">
                {item.title}
              </h3>
...
function Certifications({ language }: { language: ContentLanguage }) {
  ...
  return (
    ...
    >
      <div className="site-container">
        <h2 className="text-center font-[var(--font-roboto-condensed)] text-[36px] font-bold leading-[1.12] tracking-normal text-[#0a0a0b] sm:text-[44px]">
        ...
        <div className="mt-10 flex gap-0 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden lg:grid lg:grid-cols-6">
          {certifications.map((item, index) => (
            ...
            >
              ...
              <h3 className="mt-4 max-w-[210px] font-[var(--font-roboto-condensed)] text-[18px] font-bold leading-[1.25] text-[#1a1a1a]">
                {item.title}
              </h3>
            </article>
          ))}
...
export default async function Home() {
  ...
  const homeStories: HomeStoryCard[] = MediaData.map((story, index) => ({
    id: `media-coverage-${index + 1}`,
    title: story.title,
    excerpt: story.desc,
    logoHeight: story.height,
    logoSrc: story.imageSrc,
    logoWidth: story.width,
    href: story.link,
  }));

  try {
    const [dbIndustries, dbProducts, sourceIndustries, sourceProducts] = await Promise.all([
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-white text-[#01060a]">
      ...
      <HomeMediaCoverageSection
        heading={getMessages(language).home.storiesHeading}
        readMoreLabel={getMessages(language).common.readMore}
        stories={homeStories}
      />
      <AnimatedTestimonialsSection
